...
public interface CourierMapper extends BaseMapper<Courier> {
     ...
     */
    @Select("SELECT c.*, " +
            "6378.137 * 2 * ASIN(SQRT(POW(SIN((#{latitude} * PI() / 180 - SUBSTRING_INDEX(c.service_area, ',', -2) * PI() / 180) / 2), 2) + " +
            "COS(#{latitude} * PI() / 180) * COS(SUBSTRING_INDEX(c.service_area, ',', -2) * PI() / 180) * " +
            "POW(SIN((#{longitude} * PI() / 180 - SUBSTRING_INDEX(c.service_area, ',', -1) * PI() / 180) / 2), 2))) AS distance " +
            "FROM t_courier c " +
            "WHERE c.audit_status = 1 AND c.service_status = 1 " +
            "HAVING distance <= #{distance} " +
            "ORDER BY distance")
    IPage<Courier> findNearby(Page<Courier> page, 
     ...
     * @return 影响行数
     */
    @Select("UPDATE t_courier SET rating = #{rating} WHERE id = #{courierId}")
     ...
     * @return 影响行数
     */
    @Select("UPDATE t_courier SET completed_orders = completed_orders + 1 WHERE id = #{courierId}")
     ...
     * @return 影响行数
     */
    @Select("UPDATE t_courier SET service_status = #{serviceStatus} WHERE id = #{courierId}")
